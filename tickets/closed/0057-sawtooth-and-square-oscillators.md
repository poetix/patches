...
- [ ] `SawtoothOscillator::new(base_voct: f64)` and `SquareOscillator::new(base_voct: f64)` compile.
...
- [ ] Frequency computed as `C2_FREQ * 2_f64.powf(base_voct + inputs[0])` each sample.
...
`C2_FREQ = 65.406_194_f64` (Hz). Define as a module-level constant in `waveforms.rs`.

Both oscillators share the same phase accumulation logic; consider a private
`advance_phase(phase: &mut f64, freq: f64, sample_rate: f64)` helper in the same file
to avoid duplication, or just inline it — both options are fine.

`2_f64.powf(x)` is in `std`; no new dependency needed.
